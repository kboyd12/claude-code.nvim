...
describe('keymaps', function()
  ...
  describe('register_keymaps', function()
    ...
    it('should register shift+enter keymap when enabled', function()
      -- Setup claude_code table with current_instance and instances
      claude_code.claude_code = {
        current_instance = 'test_instance',
        instances = { test_instance = 42 },
      }

      -- Enable shift_enter
      config.keymaps.shift_enter = true
      ...
      -- Check if shift+enter keymap was registered
      local shift_enter_found = false
      for _, mapping in ipairs(mapped_keys) do
        if mapping.lhs == '<S-CR>' and mapping.mode == 't' then
          shift_enter_found = true
          assert.are.equal('<CR>', mapping.rhs)
          assert.are.equal('Insert newline', mapping.opts.desc)
          break
        end
      end

      assert.is_true(shift_enter_found, 'Shift+Enter keymap should be registered when enabled')
    end)

    it('should not register shift+enter keymap when disabled', function()
      -- Setup claude_code table with current_instance and instances
      claude_code.claude_code = {
        current_instance = 'test_instance',
        instances = { test_instance = 42 },
      ...
      -- Disable shift_enter
      config.keymaps.shift_enter = false
      ...
      -- Check if shift+enter keymap was NOT registered
      local shift_enter_found = false
      for _, mapping in ipairs(mapped_keys) do
        if mapping.lhs == '<S-CR>' and mapping.mode == 't' then
          shift_enter_found = true
      ...
      assert.is_false(
        shift_enter_found,
        'Shift+Enter keymap should not be registered when disabled'
      )
    end)
  end)
end)
